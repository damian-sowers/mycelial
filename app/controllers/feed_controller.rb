class FeedController < ApplicationController
	...
	def index
		...
		if params[:tag]
			...
			if @tag
				...
				@projects = get_tag_specific_projects(@tag.id, get_projects_per_page())
			else 
				@total_projects = 0
			...
			@projects = get_projects(get_projects_per_page())
		end
	...
	def load_more
		...
		if params[:tag] && !params[:tag].empty?
			...
			if @tag
				@projects = get_tag_specific_projects(@tag.id, get_projects_per_page(), offset)
			else 
				#need to include a message here saying no projects were found with this tag name
				@projects = []
			end
		else 
			@projects = get_projects(get_projects_per_page(), offset)
		end
